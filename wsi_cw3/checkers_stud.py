...
class Board:
            ...
            for col in range((row+1) % 2, BOARD_WIDTH, 2):
                if not self.board[row][col].is_empty():
                    if self.board[row][col].is_white():
                        h -= self.board[row][col].is_king()*9 + 1
                    else:
                        h += self.board[row][col].is_king()*9 + 1
        return h
    ...
    def no_figures_end(self):
        return self.white_fig_left==0 or self.blue_fig_left==0
...
def minimax_a_b(board, depth, blue_move):
    moves = board.get_possible_moves(blue_move)
    if not moves:
        Raise(Exception)                                        #do zrobienia
    best_move = moves[0]
    def keyFun(move):
        board_copy = deepcopy(board)
        board_copy.make_ai_move(move)
        return minimax_a_b_recurr(board_copy, depth-1, not blue_move, -1000, 1000, blue_move)   # max = blue, min = white
    if blue_move:
        best_move = max(moves, key=keyFun)
    else:
        best_move = min(moves, key=keyFun)
    return best_move

def minimax_a_b_recurr(board, depth, move_max, a, b, is_blue):
    if depth == 0:
        if is_blue:
            return board.evaluate3()
        else:
            return board.evaluate3()                            # TUTAJ EVALUATE
    moves = board.get_possible_moves(move_max)
    if move_max:
        for move in moves:
            board_copy = deepcopy(board)
            board_copy.make_ai_move(move)
            a = max(a, minimax_a_b_recurr(board_copy, depth-1, not move_max, a, b, is_blue))
            if a >= b:
                return a
        return a
    else:
        for move in moves:
            board_copy = deepcopy(board)
            board_copy.make_ai_move(move)
            b = min(b, minimax_a_b_recurr(board_copy, depth-1, not move_max, a, b, is_blue))
            if a >= b:
                return b
        return b
...
def main():
    ...
    while is_running:
        ...
        if game.board.white_turn:
            move = minimax_a_b(deepcopy(game.board), 2, False)      #WHITE
            game.board.make_ai_move(move)
        else:
            move = minimax_a_b(deepcopy(game.board), 5, True)       #BLUE
            game.board.make_ai_move(move)
